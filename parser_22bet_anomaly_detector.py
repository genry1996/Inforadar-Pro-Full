...
TELEGRAM_MIN_ALERT_THRESHOLD = float(os.getenv("TELEGRAM_MIN_ALERT_THRESHOLD", "2.0"))

# Инициализируем Telegram бота
telegram_notifier = None
if TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID:
    telegram_notifier = TelegramNotifier(TELEGRAM_BOT_TOKEN, TELEGRAM_CHAT_ID)
    logger.info(f"📱 Telegram уведомления активированы (порог: ≥{TELEGRAM_MIN_ALERT_THRESHOLD}%)")
else:
    logger.warning("⚠️ Telegram не настроен (проверьте .env)")
    ...
    async with async_playwright() as p:
        ...
        if anomalies:
            ...
            if telegram_notifier:
                # Фильтруем только критические аномалии
                critical_anomalies = [
                ...
                if critical_anomalies:
                    logger.info(f"📱 Отправляю {len(critical_anomalies)} критических аномалий в Telegram...")

                    # Если аномалий много - отправляем групповое уведомление
                    if len(critical_anomalies) > 3:
                        success = telegram_notifier.send_batch_alert(critical_anomalies)
                        if success:
                            logger.info(f"✅ Групповое уведомление отправлено ({len(critical_anomalies)} аномалий)")
                        else:
                            logger.error("❌ Не удалось отправить групповое уведомление")
                    else:
                        # Иначе отправляем по одной
                        sent_count = 0
                        for anom in critical_anomalies:
                            if telegram_notifier.send_anomaly_alert(anom):
                                sent_count += 1
                        logger.info(f"✅ Отправлено {sent_count}/{len(critical_anomalies)} уведомлений")
                else:
                    logger.info(f"ℹ️ Нет критических аномалий для Telegram (порог: ≥{TELEGRAM_MIN_ALERT_THRESHOLD}%)")
