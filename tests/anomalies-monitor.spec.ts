...
const anomaliesDrop = JSON.parse(fs.readFileSync(path.join(dataDir, 'anomalies-drop.json'), 'utf-8'));
const anomaliesCorridors = JSON.parse(fs.readFileSync(path.join(dataDir, 'anomalies-corridors.json'), 'utf-8'));
const anomaliesLimitCuts = JSON.parse(fs.readFileSync(path.join(dataDir, 'anomalies-limit-cuts.json'), 'utf-8'));
const anomaliesMixed = JSON.parse(fs.readFileSync(path.join(dataDir, 'anomalies-mixed.json'), 'utf-8'));
...
test.describe('Betting Anomalies Monitor - Core Functionality', () => {
  ...
  test('should load and display test data', async ({ page }) => {
    ...
    const icon = page.locator('.anomaly-icon').first();
...
test.describe('Betting Anomalies Monitor - Detail Modal', () => {
  ...
  test('should close modal when clicking outside', async ({ page }) => {
    ...
    const viewBtn = page.locator('button:has-text("View")').first();
    await viewBtn.click();
    const modalBackdrop = page.locator('#detailModal');
