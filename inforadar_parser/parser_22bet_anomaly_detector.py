...
# ==================== ПОВТОРНЫЙ ПАРСИНГ ОДНОГО СОБЫТИЯ ====================
        ...
        for event_name, current_data in current_events.items():
            ...
            for outcome in ['1', 'X', '2']:
                ...
                if prev_odd > 0 and curr_odd > 0:
                    ...
                    # Логируем все изменения > 5%
                    if abs(change_pct) > 5:
                        logger.info(
                            f"🔄 Изменение: {event_name} ({outcome}): "
                            f"{prev_odd:.3f} → {curr_odd:.3f} ({change_pct:+.1f}%)"
                        )

                    sport_name = current_data.get('sport', prev_data.get('sport', 'Unknown'))

                    # ПАДЕНИЕ КОЭФФИЦИЕНТА (порог -7%)
                    if change_pct < -7:
                        logger.warning(
                            ...
                            if confirmed_odds and confirmed_odds[outcome] > 0:
                                ...
                                if confirmed_change < -7:
                                    ...
                                    logger.info(
                                        ...
                                        f"кэф вернулся или изменился меньше порога"
                                    )
                            else:
                                anomalies.append({
                                    'event_name': event_name,
    ...
    async with async_playwright() as p:
        ...
        for sport in SPORTS:
            try:
                ...
                # УБРАЛИ [:20] — теперь учитываем все события
                for idx, event in enumerate(all_events):
                    try:
                        ...
                        # Отбрасываем явные технические события
                        ...
                        try:
                            sql = """
                                INSERT INTO odds_22bet (
                                    event_name, sport, market_type,
                                    odd_1, odd_x, odd_2, status
                                )
                ...
                anomalies = await detector.detect_anomalies_with_confirmation(
                    current_events, page
                )
                    ...
                    for anom in anomalies:
                        try:
                            sql = """
                                ...
                                (event_name, sport, league, anomaly_type,
                                 before_value, after_value, diff_pct,
                                 status, comment)
                            ...
                            logger.warning(
                                f"  📍 {anom['anomaly_type']}: "
                                f"{anom['event_name']} [{anom['status']}]"
                            )

                            # Telegram: только подтвержденные падения
                            if (
                                anom['anomaly_type'] == 'ODDS_DROP'
                                and anom['status'] == 'confirmed'
                            ):
                                anom['league'] = 'Mixed'
                ...
                logger.info(
                    f"✅ {sport['name']}: {success_count} событий, "
