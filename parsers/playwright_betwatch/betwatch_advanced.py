# 🎯 BETWATCH ADVANCED DETECTOR v2.1
# Динамические пороги + Telegram + Звуковые алерты
...
# ============ ЗАГРУЗКА КАСТОМНЫХ ПОРОГОВ ============
def load_custom_thresholds():
    """Загрузка порогов из JSON файла (если есть)"""
                                ...
                                if sharp_drop:
                                    ...
                                    if should_send_signal(signal_key, reported_signals):
                                        ...
                                        text = (
                                            ...
                                            f"🏆 {league}\n"
                                            f"💰 Market: {bet_type}\n\n"
                                            f"{sharp_drop['old_odd']:.2f} → {sharp_drop['new_odd']:.2f}\n"
                                            f"Drop: {sharp_drop['drop_percent']:.1f}%\n"
                                            f"Money: €{money:,.0f}\n\n"
...
async def main():
    logger.info("=" * 80)
    logger.info("🎯 BETWATCH ADVANCED DETECTOR v2.1")
    logger.info("=" * 80)
    
    # Загружаем кастомные пороги
