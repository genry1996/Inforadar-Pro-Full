...
def upsert_match(cur, event_id, sport, league, team1, team2, start_ts, is_live):
    cur.execute(
        """
        ...
        ON DUPLICATE KEY UPDATE
            ...
            updated_at=NOW()
        """,
        (event_id, sport, league, team1, team2, start_ts, is_live),
    )

def upsert_odds(cur, event_id, market_key, outcome_key, odd, limit_val):
    cur.execute(
        """
        ...
        ON DUPLICATE KEY UPDATE
            ...
            updated_at=NOW()
        """,
        (event_id, market_key, outcome_key, odd, limit_val),
    )
    cur.execute(
        """
